...
class Index extends PureComponent {
    render() {
        const { loading, p1, dispatch,dict,titleText } = this.props;
        const onSubmit = (data) => {
        ...
        return (
            <Page title={'近十年一个单位中标统计'} loading={loading}>
                <View  titleText={`${titleText}近十年的中标情况`} data={p1} showY2={false} Y2SeriesIndex={[1]} Y2Name={"万"} YName={"万"} handleSubmit={onSubmit} loading={loading} dict={dict} />
            </Page>
        );
    }
}
